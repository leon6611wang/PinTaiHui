...
public class CartGoodsNormsRecyclerAdapter extends RecyclerView.Adapter<CartGoodsNormsRecyclerAdapter.ViewHolder> implements OnShangPinInformationGuiGePiPeiListener {
    private Context context;
    private int dp_15;
    private List<GoodsNormGroup> guiGeList;
    private Map<Integer, GoodsNorm> map = new HashMap<>();
    private List<Integer> pipeiList;
    private CartGoodsNormsDialog dialog;
    private MyHandler myHandler;

    private static class MyHandler extends Handler {
        WeakReference<CartGoodsNormsRecyclerAdapter> shangPinInformationGuiGeRecyclerAdapterWeakReference;

        public MyHandler(CartGoodsNormsRecyclerAdapter adapter) {
            shangPinInformationGuiGeRecyclerAdapterWeakReference = new WeakReference<>(adapter);
        }

        @Override
        public void handleMessage(Message msg) {
            CartGoodsNormsRecyclerAdapter adapter = shangPinInformationGuiGeRecyclerAdapterWeakReference.get();
            switch (msg.what) {
                case 1:
...
//                    ArrayList<Integer> list=bundle.getIntegerArrayList("list");
    ...
    public CartGoodsNormsRecyclerAdapter(Context context, CartGoodsNormsDialog d) {
