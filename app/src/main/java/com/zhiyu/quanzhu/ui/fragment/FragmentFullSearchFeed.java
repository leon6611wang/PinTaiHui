 ...
 * 全局搜索-动态
 */
public class FragmentFullSearchFeed extends Fragment {
    private View view;
    private PtrFrameLayout ptrFrameLayout;
    private ListView mListView;
    private FullSearchFeedListAdapter adapter;
    private MyHandler myHandler = new MyHandler(this);

    private static class MyHandler extends Handler {
        WeakReference<FragmentFullSearchFeed> fullSearchFeedWeakReference;

        public MyHandler(FragmentFullSearchFeed fragment) {
            fullSearchFeedWeakReference = new WeakReference<>(fragment);
        }

        @Override
        public void handleMessage(Message msg) {
            FragmentFullSearchFeed fragment = fullSearchFeedWeakReference.get();
            switch (msg.what) {
                case 1:
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_full_search_feed, null);
        initViews();
        return view;
    }
    ...
    private void initViews() {
        ...
        adapter = new FullSearchFeedListAdapter(getActivity(),getContext());
    ...
    private FullSearchFeedResult feedResult;
    private void search() {
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
            ...
            public void onSuccess(String result) {
                System.out.println("feed: "+result);
                feedResult= GsonUtils.GsonToBean(result,FullSearchFeedResult.class);
                if(isRefresh){
                    list=feedResult.getData().getFeeds_list();
                }else{
                    list.addAll(feedResult.getData().getFeeds_list());
                }
                Message message=myHandler.obtainMessage(1);
                message.sendToTarget();
                System.out.println("full search feed: " + feedResult.getData().getFeeds_list().size());
            }

            @Override
            public void onError(Throwable ex, boolean isOnCallback) {
                System.out.println("full search feed: " + ex.toString());
            }
