...
import com.zhiyu.quanzhu.model.result.PointExchangeRecordResult;
import com.zhiyu.quanzhu.ui.adapter.PointExchangeRecordRecyclerAdapter;
...
public class PointExchangeRecordActivity extends BaseActivity implements View.OnClickListener {
    ...
    private PointExchangeRecordRecyclerAdapter adapter;

    private static class MyHandler extends Handler {
        WeakReference<PointExchangeRecordActivity> huiYuanDuiHuanJiLuActivityWeakReference;

        public MyHandler(PointExchangeRecordActivity activity) {
            huiYuanDuiHuanJiLuActivityWeakReference = new WeakReference<>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            PointExchangeRecordActivity activity = huiYuanDuiHuanJiLuActivityWeakReference.get();
            switch (msg.what) {
                case 1:
    ...
    private void initViews() {
        ...
        adapter = new PointExchangeRecordRecyclerAdapter(this);
    ...
    private PointExchangeRecordResult recordResult;
    private List<PointExchangeRecord> list;
    private void pointExchangeRecordList() {
        RequestParams params = MyRequestParams.getInstance(this).getRequestParams(ConstantsUtils.BASE_URL + ConstantsUtils.POINT_EXCHANGE_RECORD_LIST);
        params.addBodyParameter("page", String.valueOf(page));
        x.http().post(params, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                System.out.println("exchange: "+result);
                recordResult= GsonUtils.GsonToBean(result,PointExchangeRecordResult.class);
                if(isRefresh){
            ...
            public void onError(Throwable ex, boolean isOnCallback) {
                System.out.println("exchange: "+ex.toString());
            }
