 ...
 * 首页-圈子-关注列表适配器
 */
public class MyCollectionArticleAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private boolean isSelectModel;//可选状态
    ...
    public MyCollectionArticleAdapter(Activity aty, final Context context) {
    ...
    private static class MyHandler extends Handler {
        WeakReference<MyCollectionArticleAdapter> adapterWeakReference;

        public MyHandler(MyCollectionArticleAdapter adapter) {
            adapterWeakReference = new WeakReference<>(adapter);
        }

        @Override
        public void handleMessage(Message msg) {
            MyCollectionArticleAdapter adapter = adapterWeakReference.get();
            switch (msg.what) {
                case 1:
    ...
    class ArticleViewHolder extends RecyclerView.ViewHolder {
        ...
        HorizontalListView tagListView;
        CircleGuanZhuFeedTagListAdapter adapter = new CircleGuanZhuFeedTagListAdapter();
        ImageView collectImageView, priseImageView, followImageView, selectImageView;
...
//                Glide.with(context).load(list.get(position).getContent().getVideo_url()).apply(BaseApplication.getInstance().getVideoCoverImageOption()).apply(BaseApplication.getInstance().getVideoCoverImageOption()).into(feed.videoPlayer.getCoverController().getVideoCover());
            ...
            if (list.get(position).getContent().isIs_follow()) {
                article.followLayout.setBackground(context.getResources().getDrawable(R.drawable.shape_oval_solid_bg_ededed_gray));
                article.followImageView.setVisibility(View.GONE);
                article.followTextView.setText("已关注");
                article.followTextView.setTextColor(context.getResources().getColor(R.color.text_color_gray));
            } else {
            ...
            if (isSelectModel) {
                ...
                if (list.get(position).isSelected()) {
                    article.selectImageView.setImageDrawable(context.getResources().getDrawable(R.mipmap.gouwuche_selected));
                } else {
                    article.selectImageView.setImageDrawable(context.getResources().getDrawable(R.mipmap.gouwuche_unselect));
                }
            } else {
                article.selectLayout.setVisibility(View.GONE);
            }
            article.selectLayout.setOnClickListener(new OnSelectClick(position));
