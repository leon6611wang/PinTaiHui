...
public class FeedInformationActivity extends BaseActivity implements View.OnClickListener
        , FeedInfoCommentsParentRecyclerAdapter.OnReplySuccessListener {
    private MyLocationRecyclerView mRecyclerView;
    private FeedInfoCommentsParentRecyclerAdapter adapter;
...
//    private int feed_header_type;//1.image,2.video
    ...
    private static class MyHandler extends Handler {
        WeakReference<FeedInformationActivity> activityWeakReference;
        ...
        public void handleMessage(Message msg) {
            FeedInformationActivity activity = activityWeakReference.get();
            switch (msg.what) {
                case 1:
...
//                            parent.setAdapter_type(activity.feed_header_type);
                    ...
                    break;
                case 2:
                    if (activity.feedInfoResult.getCode() == 200) {
...
//                            parent.setAdapter_type(activity.feed_header_type);
                        ...
                        Toast.makeText(activity, activity.feedInfoResult.getMsg(), Toast.LENGTH_SHORT).show();
                    }
                    break;
                case 3:
                ...
                case 4://collect
                    if (activity.baseResult.getCode() == 200) {
                        if (!activity.feedInfoResult.getData().getDetail().isIs_collect()) {
                            activity.feedInfoResult.getData().getDetail().setIs_collect(true);
                            Toast.makeText(activity, "收藏成功", Toast.LENGTH_SHORT).show();
                            activity.collectImageView.setImageDrawable(activity.getResources().getDrawable(R.mipmap.collect_yellow_big));
                            activity.collectNumTextView.setVisibility(View.VISIBLE);
                            activity.feedInfoResult.getData().getDetail().setCollect_num(activity.feedInfoResult.getData().getDetail().getCollect_num() + 1);
                            activity.collectNumTextView.setText(String.valueOf(activity.feedInfoResult.getData().getDetail().getCollect_num()));
                        } else {
                            activity.feedInfoResult.getData().getDetail().setIs_collect(false);
                            activity.collectImageView.setImageDrawable(activity.getResources().getDrawable(R.mipmap.collect_gray_big));
                            activity.feedInfoResult.getData().getDetail().setCollect_num(activity.feedInfoResult.getData().getDetail().getCollect_num() - 1);
                            activity.collectNumTextView.setText(String.valueOf(activity.feedInfoResult.getData().getDetail().getCollect_num()));
                            if (activity.feedInfoResult.getData().getDetail().getCollect_num() <= 0) {
                ...
                case 5://prise
                    if (activity.baseResult.getCode() == 200) {
                        if (!activity.feedInfoResult.getData().getDetail().isIs_prise()) {
                            activity.feedInfoResult.getData().getDetail().setIs_prise(true);
                            Toast.makeText(activity, "点赞成功", Toast.LENGTH_SHORT).show();
                            activity.priseImageView.setImageDrawable(activity.getResources().getDrawable(R.mipmap.prise_yellow_big));
                            activity.priseNumTextView.setVisibility(View.VISIBLE);
                            activity.feedInfoResult.getData().getDetail().setPrise_num(activity.feedInfoResult.getData().getDetail().getPrise_num() + 1);
                            activity.priseNumTextView.setText(String.valueOf(activity.feedInfoResult.getData().getDetail().getPrise_num()));
                        } else {
                            activity.feedInfoResult.getData().getDetail().setIs_prise(false);
                            activity.priseImageView.setImageDrawable(activity.getResources().getDrawable(R.mipmap.prise_gray_big));
                            activity.feedInfoResult.getData().getDetail().setPrise_num(activity.feedInfoResult.getData().getDetail().getPrise_num() - 1);
                            activity.priseNumTextView.setText(String.valueOf(activity.feedInfoResult.getData().getDetail().getPrise_num()));
                            if (activity.feedInfoResult.getData().getDetail().getPrise_num() <= 0) {
...
//        videoView = LayoutInflater.from(this).inflate(R.layout.item_dongtai_information_header_text_video, null);
        adapter = new FeedInfoCommentsParentRecyclerAdapter(this, mRecyclerView);
