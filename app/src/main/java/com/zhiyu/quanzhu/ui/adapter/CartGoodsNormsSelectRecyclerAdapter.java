...
public class CartGoodsNormsSelectRecyclerAdapter extends RecyclerView.Adapter<CartGoodsNormsSelectRecyclerAdapter.ViewHolder> implements OnShangPinInformationGuiGePiPeiListener {
    private Context context;
    private int dp_15;
    private List<GoodsNormGroup> guiGeList;
    private Map<Integer, GoodsNorm> map = new HashMap<>();
    private List<Integer> pipeiList;
    private CartGoodsNormsSelectDialog dialog;
    private MyHandler myHandler;


    private static class MyHandler extends Handler {
        WeakReference<CartGoodsNormsSelectRecyclerAdapter> shangPinInformationGuiGeRecyclerAdapterWeakReference;

        public MyHandler(CartGoodsNormsSelectRecyclerAdapter adapter) {
            shangPinInformationGuiGeRecyclerAdapterWeakReference = new WeakReference<>(adapter);
        }

        @Override
        public void handleMessage(Message msg) {
            CartGoodsNormsSelectRecyclerAdapter adapter = shangPinInformationGuiGeRecyclerAdapterWeakReference.get();
            switch (msg.what) {
                case 1:
...
//                    ArrayList<Integer> list=bundle.getIntegerArrayList("list");
    ...
    public void setGroupList(List<GoodsNormGroup> list, int position) {
        guiGeList = list;
        notifyItemChanged(position);
    }

    public CartGoodsNormsSelectRecyclerAdapter(Context context, CartGoodsNormsSelectDialog d) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_goods_norms, null, false));
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int index) {
        holder.nameTextView.setText(guiGeList.get(index).getName());
