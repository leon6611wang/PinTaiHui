...
public class GoodsNormsRecyclerAdapter extends RecyclerView.Adapter<GoodsNormsRecyclerAdapter.ViewHolder> implements OnShangPinInformationGuiGePiPeiListener {
    private Context context;
    private int dp_15;
    private List<GoodsNormGroup> guiGeList;
    private Map<Integer, GoodsNorm> map = new HashMap<>();
    private List<Integer> pipeiList;
    private GoodsNormsDialog dialog;
    private MyHandler myHandler;

    private static class MyHandler extends Handler {
        WeakReference<GoodsNormsRecyclerAdapter> shangPinInformationGuiGeRecyclerAdapterWeakReference;

        public MyHandler(GoodsNormsRecyclerAdapter adapter) {
            shangPinInformationGuiGeRecyclerAdapterWeakReference = new WeakReference<>(adapter);
        }

        @Override
        public void handleMessage(Message msg) {
            GoodsNormsRecyclerAdapter adapter = shangPinInformationGuiGeRecyclerAdapterWeakReference.get();
            switch (msg.what) {
                case 1:
...
//                    ArrayList<Integer> list=bundle.getIntegerArrayList("list");
    ...
    public GoodsNormsRecyclerAdapter(Context context, GoodsNormsDialog d) {
