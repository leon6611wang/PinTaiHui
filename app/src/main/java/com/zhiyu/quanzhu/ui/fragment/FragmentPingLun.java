...
import com.zhiyu.quanzhu.model.result.FooterPrintCommentResult;
...
public class FragmentPingLun extends Fragment {
    ...
    private static class MyHandler extends Handler {
        WeakReference<FragmentPingLun> fragmentWeakReference;

        public MyHandler(FragmentPingLun fragment) {
            fragmentWeakReference = new WeakReference<>(fragment);
        }

        @Override
        public void handleMessage(Message msg) {
            FragmentPingLun fragment = fragmentWeakReference.get();
            switch (msg.what) {
                case 1:
    ...
    private void initViews() {
        ...
        adapter = new PingLunRecyclerAdapter(getContext());
    ...
    private FooterPrintCommentResult commentResult;
    private List<FooterPrintComment> list;

    private void pinglunList() {
        RequestParams params = MyRequestParams.getInstance(getContext()).getRequestParams(ConstantsUtils.BASE_URL + ConstantsUtils.MY_COMMENTS_LIST);
        params.addBodyParameter("page", String.valueOf(page));
        x.http().post(params, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                System.out.println("我的足迹-评论: " + result);
                commentResult = GsonUtils.GsonToBean(result, FooterPrintCommentResult.class);
                if (isRefresh) {
