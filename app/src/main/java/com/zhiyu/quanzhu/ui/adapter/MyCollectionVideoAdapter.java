...
public class MyCollectionVideoAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MyCollectionVideoAdapter(Activity aty, final Context context) {
    ...
    private static class MyHandler extends Handler {
        WeakReference<MyCollectionVideoAdapter> adapterWeakReference;

        public MyHandler(MyCollectionVideoAdapter adapter) {
            adapterWeakReference = new WeakReference<>(adapter);
        }

        @Override
        public void handleMessage(Message msg) {
            MyCollectionVideoAdapter adapter = adapterWeakReference.get();
            switch (msg.what) {
                case 1:
                ...
                case 3:
                    ...
                    if (adapter.baseResult.getCode() == 200) {
                        int posiiton = (Integer) msg.obj;
                        adapter.list.get(posiiton).getContent().setIs_follow(!adapter.list.get(posiiton).getContent().isIs_follow());
                        adapter.notifyDataSetChanged();
                    }
                    break;
            }
    ...
    class ArticleViewHolder extends RecyclerView.ViewHolder {
        ...
        CircleImageView avatarImageView;
        TextView nameTextView, titleTextView, sourceTextView, timeTextView, shareTextView, commentTextView, priseTextView, followTextView;
        LinearLayout priseLayout, followLayout;
        NiceImageView coverImageView;
        HorizontalListView tagListView;
        CircleGuanZhuFeedTagListAdapter adapter = new CircleGuanZhuFeedTagListAdapter();
        ImageView collectImageView, priseImageView, followImageView;
        RelativeLayout closeLayout;
    ...
    class VideoViewHolder extends RecyclerView.ViewHolder {
        CircleImageView avatarImageView;
        TextView nameTextView, timeTextView, sourceTextView, shareTextView, commentTextView, priseNumTextView, followTextView;
        ImageView collectImageView, priseImageView, followImageView;
        ExpandableTextView mTextView;
        HorizontalListView tagListView;
        CircleGuanZhuFeedTagListAdapter adapter = new CircleGuanZhuFeedTagListAdapter();
        VideoPlayerTrackView videoPlayer;
        LinearLayout priseLayout, itemRootLayout, followLayout;
        RelativeLayout closeLayout, selectLayout;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == FEED) {
            ...
            return new ArticleViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_my_collection_article, parent, false));
        } else if (viewType == VIDEO) {
            return new VideoViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_my_collection_video, parent, false));
        }
        return null;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof FeedViewHolder) {
            ...
            if (list.get(position).getContent().isIs_follow()) {
                article.followLayout.setBackground(context.getResources().getDrawable(R.drawable.shape_oval_solid_bg_gray));
                article.followImageView.setVisibility(View.GONE);
                article.followTextView.setText("已关注");
                article.followTextView.setTextColor(context.getResources().getColor(R.color.white));
            } else {
            ...
            article.followLayout.setOnClickListener(new OnFollowClick(position));
            ...
            if (list.get(position).getContent().isIs_follow()) {
                video.followLayout.setBackground(context.getResources().getDrawable(R.drawable.shape_oval_solid_bg_ededed_gray));
                video.followImageView.setVisibility(View.GONE);
                video.followTextView.setText("已关注");
                video.followTextView.setTextColor(context.getResources().getColor(R.color.text_color_gray));
            } else {
            ...
            if (isSelectModel) {
                ...
                if (list.get(position).isSelected()) {
                    video.selectImageView.setImageDrawable(context.getResources().getDrawable(R.mipmap.gouwuche_selected));
                } else {
                    video.selectImageView.setImageDrawable(context.getResources().getDrawable(R.mipmap.gouwuche_unselect));
                }
            } else {
                video.selectLayout.setVisibility(View.GONE);
            }
            video.selectLayout.setOnClickListener(new OnSelectClick(position));
            video.followLayout.setOnClickListener(new OnFollowClick(position));
...
//            Intent commentIntent=new Intent(context,Comment)
    ...
    private class OnFollowClick implements View.OnClickListener {
        ...
        public OnFollowClick(int position) {
            this.position = position;
        }
