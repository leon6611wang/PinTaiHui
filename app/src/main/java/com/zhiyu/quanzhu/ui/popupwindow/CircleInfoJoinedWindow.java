 ...
 * 商圈详情-右上角菜单-已加入
 */
public class CircleInfoJoinedWindow extends PopupWindow implements View.OnClickListener {
    private Context context;
    private View view;
    private LinearLayout shareLayout,messageLayout,complaintLayout,exitLayout;
    public CircleInfoJoinedWindow(Context context,OnMenuSelectListener listener) {
        super(context);
    ...
    private void initalize() {
        ...
        view = inflater.inflate(R.layout.popupwindow_circle_info_joined, null);
        setContentView(view);
        initWindow();
        initViews();
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.messageLayout:
                if(null!=onMenuSelectListener){
                    onMenuSelectListener.onMenuSelect(2,"消息");
                }
                dismiss();
                break;
            case R.id.complaintLayout:
                if(null!=onMenuSelectListener){
                    onMenuSelectListener.onMenuSelect(3,"投诉");
                }
                dismiss();
                break;
            case R.id.exitLayout:
                if(null!=onMenuSelectListener){
                    onMenuSelectListener.onMenuSelect(4,"退出");
                }
                dismiss();
                break;
            default:
